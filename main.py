...
# ==========================================
# 0. 사운드 컨트롤 사이드바 (볼륨 조절 삭제)
# ==========================================
with st.sidebar:
    st.header("⚙️ 사운드 컨트롤")
    st.markdown("시각 장애인의 청각 예민도에 맞춰 설정하세요.")
    
    # 주파수 상한선 조절만 유지 (기본 1800Hz에서 최대 5000Hz까지 확장 가능)
    max_freq_input = st.slider("최대 주파수 설정 (Hz)", 1000, 5000, 1800, step=100)
    
    st.info(f"현재 설정: 200Hz ~ {max_freq_input}Hz")

# ==========================================
# 1. 핵심 사운드 변환 엔진 (user_vol 파라미터 삭제)
# ==========================================
def generate_stereo_sound(data_values, col_name, user_max_f): # user_vol 삭제됨
...
with tab1:
    ...
    uploaded_csv = st.file_uploader("CSV 파일을 올려주세요", type=['csv'], key="csv_uploader")
...
with tab2:
    ...
    uploaded_image = st.file_uploader("그래프 영역 이미지를 올려주세요", type=['png', 'jpg', 'jpeg'], key="img_uploader")
